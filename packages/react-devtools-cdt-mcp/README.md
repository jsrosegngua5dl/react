...
- **Output** — every tool returns a [TOON](https://github.com/toon-format/toon)
  string (a compact, token-efficient encoding) of the shape described below. On
  failure a tool returns `{error: string}` instead.
...
- **Output:** `{label, type, name, key?, props?, hooks?}`. `props` excludes
  ...
  components only) is an array of `{id, name, value, subHooks}`.
...
- **Output:** `{page, pageSize, totalCount, totalPages, results}` where
  `results` is an array of tree nodes (same shape as `react_get_tree`).
