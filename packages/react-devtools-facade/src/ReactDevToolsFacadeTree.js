...
import type {RendererInternals} from './ReactDevToolsFacade';
...
export function createTreeTools(
  ...
  function getComponentByLabel(label: string): NodeInfo | ToolError {
    ...
    // simple-memo components. inspectHooksOfFiber re-renders the component, so
    // guard by tag and tolerate failures (e.g. a component that throws).
    ...
    if (
      ...
      try {
        const hooksTree = inspectHooksOfFiber(
          fiber,
   ...
   * getComponentByLabel, getComponentSource, getTreeSnapshot).
   *
