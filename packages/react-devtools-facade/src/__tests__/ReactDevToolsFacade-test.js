...
describe('react-devtools-facade', () => {
  ...
  describe('multiple roots and renderers', () => {
    ...
    it('getTreeSnapshot aggregates components from multiple roots', () => {
      function AppA() {
        return <div>A</div>;
      }
      function AppB() {
      ...
      try {
        ...
        const tree = createTools(facade).getTreeSnapshot();
        ...
        // Labels are globally unique across roots.
        expect(appA.label).not.toBe(appB.label);
      } finally {
    ...
    it('findComponents finds matches across multiple roots', () => {
      ...
      try {
        ...
        expect(result.results[0].label).not.toBe(result.results[1].label);
      } finally {
        document.body.removeChild(containerB);
      }
    });
